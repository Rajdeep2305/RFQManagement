...
const Rfqcr = () => {
  ...
  const handleDelete = async (rfqID) => {
    try {
      const response = await fetch(`${backendUrl}/api/rfq/deleteRfq`, {
        method: "POST", // Changed method to POST
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ rfqID }), // Send the rfqID in the request body
      });
      ...
      if (response.ok) {
        ...
        setData((prevData) => prevData.filter((item) => item.rfqID !== rfqID));
      } else {
  ...
  return (
    <div className="pt-4 px-2 w-full">
      <div className="pt-7 pl-6">
        <p className="text-[26px]">Batch Management of RFQs</p>
      </div>
                    ...
                    >
                      <>
                        ...
                        <p className="mt-2 text-gray-600">
                          Submission Date:{" "}
                          <strong>{submissionDate.toLocaleString()}</strong>
                        </p>
                        ...
                        <div className="mt-4 text-center">
                          <button
                            onClick={() => handleDelete(item.rfqID)}
